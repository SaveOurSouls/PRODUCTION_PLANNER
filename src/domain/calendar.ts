...
export function fitWork(calendars:Calendar[],busy:Span[],earliest:number,duration:number,horizon=730):WorkSegment[]|null {
  ...
  for(let i=0;i<horizon&&remaining>0.00001;i++) {
    ...
    let spans=daySpans(calendars[0],date);
    for(const c of calendars.slice(1)) {
      ...
      const resourceSpans=daySpans(c,first.toISODate()!);
      spans=intersect(spans,first.toISODate()===last.toISODate()?resourceSpans:[...resourceSpans,...daySpans(c,last.toISODate()!)]);
    }
